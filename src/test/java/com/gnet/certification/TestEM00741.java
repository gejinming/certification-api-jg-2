...
public class TestEM00741 extends CertificationAddBeforeJUnit {
...
//		data.put("eduClassId", 302649);
		data.put("eduClassId", 196286);
//		HttpResponse httpResponse = super.api("EM00741", token, data).send();
		String fileUrl = "F:\\_Use_One\\input\\student3-y-2.xls";
		File file = new File(fileUrl);
		HttpResponse httpResponse = super.api("EM00741", token, data).form("upfile", file).send();
		this.isOk(httpResponse);
		Response response = this.parse(httpResponse);
		JSONObject object = (JSONObject) response.getData();
		Assert.assertNotNull(object);
