...
public class EM01191 extends BaseApi implements IApi {
	...
	public Response excute(Request request, Response response, ResponseHeader header, String method) {
		...
		Page<CcCourseGroupMangeTeach> ccCourseGroupPage = CcCourseGroupMangeTeach.dao.page(pageable, planId, remark, type);
		List<CcCourseGroupMangeTeach> ccCourseGroupMangeList = ccCourseGroupPage.getList();
		// 判断是否分页
		 ...
		 */
		//
		Map<Long, List<CcCourseGroupMange>> courseGroupMap = new HashMap<>();
		if(!ccCourseGroupMangeList.isEmpty()){
			...
			for (int i = 0; i < ccCourseGroupMangeList.size(); i++) {
				CcCourseGroupMangeTeach temp = ccCourseGroupMangeList.get(i);
                courseGroupMangeIds[i] = temp.getLong("id");
			}
			// 初始化map
			for(Long id : courseGroupMangeIds) {
                courseGroupMap.put(id, new ArrayList<CcCourseGroupMange>());
			}
			List<CcCourseGroupMange> ccCoursegroupList = CcCourseGroupMange.dao.groupLists(0);
		...
		for (CcCourseGroupMangeTeach temp : ccCourseGroupMangeList) {
			...
			// 获取coursegroup中的数据
			List<CcCourseGroupMange> courseGroupList = courseGroupMap.get(temp.get("id"));
			List<CcCourseGroup> courseListReturn = new ArrayList<>();
			if(courseGroupList != null){
				for(CcCourseGroupMange courseGroup : courseGroupList) {
                    CcCourseGroup courseGroupTemp = new CcCourseGroup();
                    courseGroupTemp.set("id", courseGroup.getLong("id"));
                    courseGroupTemp.set("group_name", courseGroup.getStr("group_name"));
