...
public class EM00410 extends BaseApi implements IApi {
	...
	public Response excute(Request request, Response response, ResponseHeader header, String method) {
		...
		Page<CcCourseGradeComposeDetail> ccCourseGradeComposeDetailPage = CcCourseGradeComposeDetail.dao.page(pageable, name, detail, courseGradeComposeId,batchId);
		...
		for (CcCourseGradeComposeDetail temp : ccCourseGradeComposeDetailList) {
			...
			//循环出支持的课程目标
			List<CcCourseGradecomposeDetailIndication> indicationList = CcCourseGradecomposeDetailIndication.dao.getIndictionList(temp.getLong("id"),null);
			String indications="";
			int i=1;
			for (CcCourseGradecomposeDetailIndication temp1: indicationList){
				Integer sort = temp1.getInt("sort");
				if (i<indicationList.size()){
					indications=indications+"CO"+sort+"、";
				}else {
					indications=indications+"CO"+sort;
				}
				i++;
			}
			ccCourseGradeComposeDetail.put("indications", indications);
