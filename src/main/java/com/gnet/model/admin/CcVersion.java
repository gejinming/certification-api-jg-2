...
public class CcVersion extends DbModel<CcVersion> {
	...
	* 查找大版本下最新的小版本编号
	...
	public Long findNewestVersion(Long majorId, Integer grade) {
		...
		sql.append("order by enable_grade desc limit 1");
	 ...
	 * @date 2020/6/28 9:44
	 */
	public CcVersion getVersionState(Long planId){
		List<Object> params = new ArrayList<>();
		StringBuilder sql = new StringBuilder("select * from " + tableName + " where id=? and is_del=?");
		params.add(planId);
		params.add(DEL_NO);
		return findFirst(sql.toString(), params.toArray());
	}
