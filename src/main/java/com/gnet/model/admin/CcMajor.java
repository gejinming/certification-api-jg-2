...
public class CcMajor extends DbModel<CcMajor> {
	...
	/*
	 * @param teacherId 
	 * @return java.util.List<com.gnet.model.admin.CcMajor>
	 * @author Gejm
	 * @description: 根据老师任课的课程获取专业列表及老师所属专业
	 * @date 2020/8/5 15:03
	 */
	public List<CcMajor> findCourseMajors(Long teacherId){
		List<Object> params = Lists.newArrayList();
		//根据老师任课的课程获取专业列表
		StringBuilder sql = new StringBuilder("select  id,name majorName from ( select cm.id,so.name from cc_major cm ");
		sql.append("inner join cc_version cv on cm.id=cv.major_id and cv.is_del=0 ");
		sql.append("inner join sys_office so on so.id=cm.id and so.is_del=0 ");
		sql.append("inner join cc_plan_version cpv on cpv.id = cv.id and cpv.is_del=0 ");
		sql.append("inner join cc_course cc on cc.plan_id=cpv.id and cc.is_del=0 ");
		sql.append("inner join cc_teacher_course ctc on ctc.course_id=cc.id and ctc.is_del=0 ");
		sql.append("where ctc.teacher_id=? and cm.is_del=? ");
		params.add(teacherId);
		params.add(Boolean.FALSE);
		sql.append("group by  cm.id,so.name ");
		sql.append("union all ");
		//老师所属专业
		sql.append("select cm.id,so.name from cc_major cm ");
		sql.append("inner join cc_teacher ct on ct.major_id=cm.id and ct.is_del=0 ");
		sql.append("inner join sys_office so on so.id=cm.id and so.is_del=0 ");
		sql.append("where ct.id=? ");
		params.add(teacherId);
		sql.append(" ) a group by id,name");
