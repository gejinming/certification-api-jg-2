...
public class Record implements Serializable {
	...
	/**
     * Get seqences values from record by prefix if contains '[ ]'.
     * @param prefix
     * @return
     */
    public List<Object> getColumnValuesSeqByPrefix(String prefix) {
        List<Object> result = new ArrayList<Object>();
        Set<String> attrNameSet = getColumns().keySet();
        Iterator<String> attrNameIter = attrNameSet.iterator();
        ...
        while(attrNameIter.hasNext()) {
            String attrName = attrNameIter.next();
            Object attrValue = get(attrName);
            if(attrName.startsWith(prefix)) {
                String attrIndexStr = attrName.substring(prefix.length());
                if(StrKit.isBlank(attrIndexStr)) {
                    result.add(attrValue);
                }else{
                    Integer attrIndex = Integer.valueOf(attrIndexStr.substring(1, attrIndexStr.length() - 1));
                    CollectionKit.addItem(result, attrIndex, attrValue);
                }
            }
        }
        result = CollectionKit.removeEmptyItem(result);
        return result;
    }
