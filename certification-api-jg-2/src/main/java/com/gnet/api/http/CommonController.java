...
//		}
		...
		if(Response.WORD_FILE.equals(response.getHeader().getSuccflag()) && response.getData() != null && response.getData() instanceof File){
			...
			String targetFileName = response.getFileName();
			if (originFileName.endsWith(".docx")) {
				targetFileName += ".docx";
				httpServletResponse.setContentType("application/vnd.openxmlformats-officedocument.wordprocessingml.document");
			} else if (originFileName.endsWith(".doc")) {
				targetFileName += ".doc";
			...
			httpServletResponse.addHeader("Content-disposition", "attachment; filename=" + targetFileName);
