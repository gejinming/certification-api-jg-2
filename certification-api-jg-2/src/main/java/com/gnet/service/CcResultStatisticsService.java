...
//			return true;
	...
	public boolean statisticsCourseResultForJG(Integer grade, Long versionId) {
		...
		List<CcScoreStuIndigrade> ccScoreStuIndigrades = CcScoreStuIndigrade.dao.findAllClassGradeByEduclassIds(ccEduclassIdList);
		//List<CcEduindicationStuScore> courseScore = CcEduindicationStuScore.dao.findByEduclassIdsSumList(classList);
		...
		//目前我用课程id与总成绩组成map
		//可以用<课程id，
		//<courseId, Map<indicationId, List<Map<gradecomposeId,toalGrade>>>>
		Map<Long,BigDecimal> courseIdTotalScore=new HashMap<>();
		//课程id与
		for (CcCourse temp: ccCourseList){
			Long courseId = temp.getLong("id");
			//根据课程id查询到属于这个课程教学班List
			List<Long> classList = courseEduclassIdMap.get(courseId);
			//根据教学班List查询这个课程的按课程目标id、成绩组成id分组统计的成绩
			List<CcEduindicationStuScore> courseScore = CcEduindicationStuScore.dao.findByEduclassIdsSumList(classList);
		}*/
		...
		for(Long courseId : courseIdList) {
			...
			for(Entry<Long, Map<Long, List<Long>>> indicationCourseIdAndIndicationIdAndGradecomposeIndicationIdEntry : indicationCourseIdAndIndicationIdAndGradecomposeIndicationIdMap.entrySet()) {
				...
				if(oldCcReportCourse != null) {
					ccReportCourse.set("except_result", oldCcReportCourse.getBigDecimal("except_result"));
				}
				ccReportCourse.set("result", result.setScale(3, RoundingMode.HALF_UP));
		...
		for (Map.Entry<String, BigDecimal> entry : majorReportResultMap.entrySet()) {
			...
			// 指标点得分
			BigDecimal indicationScore = entry.getValue();
			...
			if (majorDirectionId != null && majorReportResultMap.get(noMajorDirectionIndicationKey) != null) {
				indicationScore = PriceUtils._add(indicationScore, majorReportResultMap.get(noMajorDirectionIndicationKey));
			}
			...
			if(oldCcReportMajor != null) {
				ccReportMajor.set("except_result", oldCcReportMajor.getBigDecimal("except_result"));
			}
	...
	public boolean statisticsCourseResultForJGExcept(Integer grade, Long versionId) {
        ...
        for(Long courseId : courseIdList) {
            ...
            for(Entry<Long, Map<Long, List<Long>>> indicationCourseIdAndIndicationIdAndGradecomposeIndicationIdEntry : indicationCourseIdAndIndicationIdAndGradecomposeIndicationIdMap.entrySet()) {
                ...
                ccReportCourse.set("indication_course_id", indicationCourseId);
				if(oldCcReportCourse != null) {
					ccReportCourse.set("result", oldCcReportCourse.getBigDecimal("result"));
				}
				ccReportCourse.set("except_result", result.setScale(3, RoundingMode.HALF_UP));
				ccReportCourse.set("is_del", CcReportCourse.DEL_NO);
				ccReportCourse.set("statistics_date", date);
				ccReportCourse.set("create_date", date);
				ccReportCourse.set("modify_date", date);
		...
		for (Map.Entry<String, BigDecimal> entry : majorReportResultMap.entrySet()) {
			...
			ccReportMajor.set("major_direction_id", majorDirectionId);
			if(oldCcReportMajor != null) {
				ccReportMajor.set("result", oldCcReportMajor.getBigDecimal("result"));
			}
...
//				}
		...
		// 更新之前的学生课程成绩记录
		if (!updateCcReportPersonalCourses.isEmpty() && !CcReportPersonalCourse.dao.batchUpdate(updateCcReportPersonalCourses, "result, modify_date, statistics_date, is_del")) {
