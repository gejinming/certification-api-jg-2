...
public class FileInfo {
	 ...
	 * 通过上传的文件获得文件信息
	 * 
	 * @description 传入上传的文件、目标地址、文件类型、大小信息 获取文件名和暂时路径、目标路径
	 *              将文件类型、文件名、文件暂时路径、文件目标路径、文件、最后的修改时间、文件大小设置到文件信息
	 * @param up
	 *            上传文件
	 * @return 文件信息
	 */
	public static FileInfo getFileInfo(UploadFile up, String targetUrl, FileType fileType, Date lastModifiedDate, Long size) {
		String fileName = up.getFileName();
		String tempUrl = up.getSaveDirectory() + fileName;

		targetUrl = targetUrl + UUID.randomUUID().toString() + "." + FilenameUtils.getExtension(fileName);

		return new FileInfo().setFileType(fileType).setName(fileName).setTempUrl(tempUrl).setTargetUrl(targetUrl).setFile(up.getFile()).setLastModified(lastModifiedDate).setSize(size);
	}
