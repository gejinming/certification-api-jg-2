...
public class EM00982 extends BaseApi implements IApi {
	...
	public Response excute(Request request, Response response, ResponseHeader header, String method) {
		...
		if(!ccCourseGradecomposeDetailService.validateImportSubjectScore(subjectMap, studentIds, detailIds, addList, editList, subjects, eduClassId, courseGradeComposeId, studentList, ccEduclass, subjectScoreMap)){
		...
		if(!courseGradecomposeDetailService.batchUpdateStudentGrade(courseGradecomposeIndicationIds.toArray(new Long[courseGradecomposeIndicationIds.size()]), studentIds.toArray(new Long[studentIds.size()]), detailIds.toArray(new Long[detailIds.size()]))){
			result.put("isSuccess", false);
			return renderSUC(result, response, header);
		}
