 ...
 */
@Service("EM00851")
public class EM00851 extends BaseApi implements IApi {
...
//				indicatorPointInfo.put("gradecomposeList", new ArrayList<Object>());
		...
		if(!indicationExceptValueMap.isEmpty()){
			for (Map.Entry<Long, Object> entry : result.entrySet()) {
				indicatorPointInfo = (Map<String, Object>) entry.getValue();
				indicatorPointInfo.put("indicatorPointExceptValue", indicationExceptValueMap.get(entry.getKey()) == null ? null : indicationExceptValueMap.get(entry.getKey()).setScale(3, RoundingMode.HALF_UP));
			}
		}
		return result;
	}
